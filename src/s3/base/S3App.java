...
public class S3App extends Canvas {
	...
	public static int REDRAWING_PERIOD = 2; // used to be 20
	...
	public S3App(String map, int traceInterval, List<PlayerInput> players, gatech.mmpm.tracer.ITracer a_tracer)
			throws Exception {
...
//        }
		...
		for (PlayerInput pi : players) {
			...
			if (pi.m_inputType == PlayerInput.INPUT_AI) {
				...
				if (pi.AIType.equals("ai-empty")) {
					...
					ai.gameStarts();
				} else if (pi.AIType.equals("ai-rule-based")) {
					ai = new RuleBasedAI(pi.m_playerID, m_game);
					ai.gameStarts();
				} else {
	...
	private int game_cycle() {
		...
		if (TIME_DEBUG) {
			System.out.println("Time: " + start_time + ": input " + (t1 - start_time) + " - cycle " + (t2 - t1)
					+ " - trace " + (t3 - t2) + " - actions " + (t4 - t3));
		}
	...
	/**
	 * handles end of dragging actions, and selecting all units in the given area.
	 * If multiple units are selected, only select the player's units. If multiple
	 * units are still selected, only select troops.
	 *
	 * @param current_screen_x
	 * @param current_screen_y
	 */
	public void mouseRelease(int current_screen_x, int current_screen_y) {
		if (m_state == STATE_GAME) {
			...
			// get selected units
			for (int i = map_x_start < map_x_end ? map_x_start
					: map_x_end; i < (map_x_start > map_x_end ? map_x_start : map_x_end) + 1; i++) {
				for (int j = map_y_start < map_y_end ? map_y_start
						: map_y_end; j < (map_y_start > map_y_end ? map_y_start : map_y_end) + 1; j++) {
		...
		if (m_state == STATE_GAME) {
			...
			if (current_screen_y < HUD.HUD_Y_LOC) {
				...
				if (selectedEntity != null && selectedEntity.getOwner().equals(mouse_player)) {
					...
					if (selectedEntity instanceof WPeasant && clicked != null && clicked instanceof WGoldMine) {
						if (selectedAction == null) {
							for (WUnit unit : selectedEntities) {
								selectedAction = new S3Action(unit.entityID, S3Action.ACTION_HARVEST, clicked.entityID);
								mousePlayerActions.add(selectedAction);
							}
							selectedAction = null;
							theHUD.resetButtons();
					...
					if (selectedEntity instanceof WPeasant && clicked == null) {
						...
						if (selectedAction == null && e != null) {
							if (e instanceof WOGrass) {
								for (WUnit unit : selectedEntities) {
									selectedAction = new S3Action(unit.entityID, S3Action.ACTION_MOVE, e.getX(),
											e.getY());
									mousePlayerActions.add(selectedAction);
								}
								selectedAction = null;
								...
								for (WUnit unit : selectedEntities) {
									selectedAction = new S3Action(unit.entityID, S3Action.ACTION_HARVEST, e.getX(),
											e.getY());
									mousePlayerActions.add(selectedAction);
								}
								selectedAction = null;
					...
					if (selectedEntity instanceof WTroop && clicked != null && clicked.getOwner() != null
							&& !clicked.getOwner().equals(mouse_player)) {
						if (selectedAction == null) {
							for (WUnit unit : selectedEntities) {
								selectedAction = new S3Action(unit.entityID, S3Action.ACTION_ATTACK, clicked.entityID);
								mousePlayerActions.add(selectedAction);
							}
							selectedAction = null;
							theHUD.resetButtons();
	...
	private void performActions(List<S3Action> actions) {
		for (S3Action action : actions) {
			...
			if (u != null) {
				// cat
				System.out.println(u.spriteName + "[" + action.m_targetUnit + "] -> " + action.getActionName()
						+ action.m_parameters.toString());
				// cat
