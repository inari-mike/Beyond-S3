...
public abstract class WTroop extends WUnit {
	...
	/**
	 * checks the status of the entity to see if there are any pending actions to be
	 * performed
	 */
	@Override
		...
		if (a_cycle % speed == 0) {
			if (status == null) {
				...
				case S3Action.ACTION_MOVE:
					moveTowardsTarget(a_game, (Integer) status.m_parameters.get(0),
							(Integer) status.m_parameters.get(1));
					break;
...
//		System.out.println("  My target is in " + target.getX() + "," + target.getY() + " but I want to go to " + loc.m_a + "," + loc.m_b + " I am at " + getX() + "," + getY());
	...
	protected boolean inRange(WUnit target) {
		...
		// check X
		if (x + range >= target.getX() && x - range <= target.getX() + target.getWidth() - 1) {
			// check y
			if (y + range >= target.getY() && y - range <= target.getY() + target.getLength() - 1) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Decides if the target location is within range of the unit's attack. Assumes
	 * units attack range is a square, not a circle.
	 * 
		...
		for (int xLoc = target.getX() - range; xLoc < target.getX() + target.getWidth() + range; xLoc++) {
			for (int yLoc = target.getY() - range; yLoc < target.getY() + target.getLength() + range; yLoc++) {
				...
				if (pe instanceof WOGrass) {
					double distance = Math.sqrt((xLoc - x) * (xLoc - x) + (yLoc - y) * (yLoc - y));
