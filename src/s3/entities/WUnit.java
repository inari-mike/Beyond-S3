...
public abstract class WUnit extends S3PhysicalEntity implements Comparable<WUnit> {
	// progress bars:
	public int progressTimer = 0, progressTimerMax = 0;
	...
	public WUnit(WUnit incoming) {
		...
		if (incoming.status == null) {
			...
			this.status = new S3Action(incoming.status);
		}
	...
	public void performAction(S3Action a) {
		if (status != null && a != null && status.equals(a)) {
			return;
		}
		status = a;
	...
	public void draw(Graphics2D g, int x_offset, int y_offset) {
		...
		if (target_x >= 0 && target_y >= 0) {
			g.setStroke(
					new BasicStroke(2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1f, new float[] { 5f }, 1.0f));

			g.setColor(Color.RED);
			g.drawLine((x + getWidth() / 2) * CELL_SIZE - x_offset, (y + getLength() / 2) * CELL_SIZE - y_offset,
					target_x * CELL_SIZE - x_offset, target_y * CELL_SIZE - y_offset);
			g.setStroke(new BasicStroke());
		}
		// draw progress bar:
		if (progressTimerMax != 0) {
			drawProgressBar(g, progressTimer, progressTimerMax, x_offset, y_offset);
		}
	}
	...
	public void drawProgressBar(Graphics2D g, int current, int max, int x_offset, int y_offset) {
		...
		g.setStroke(new BasicStroke(2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 1f, null, 1.0f));
		g.setColor(Color.YELLOW);
		g.drawLine(x * CELL_SIZE - x_offset, (y + getLength()) * CELL_SIZE - y_offset,
				(int) (x * CELL_SIZE - x_offset + w), (y + getLength()) * CELL_SIZE - y_offset);
		g.setColor(Color.GREEN);
		g.drawLine(x * CELL_SIZE - x_offset, (y + getLength()) * CELL_SIZE - y_offset,
				(int) (x * CELL_SIZE - x_offset + (1 - f) * w), (y + getLength()) * CELL_SIZE - y_offset);
		g.setStroke(new BasicStroke());
	}
