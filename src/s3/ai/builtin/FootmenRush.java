...
public class FootmenRush implements AI {
	...
	protected void executeRequests(List<Request> requests, S3 game, WPlayer player, List<S3Action> actions) {
		...
		// sort requests, and if the one with highest priority can be satisfied, do it:
		{
			...
			if (highest != null) {
				if (highest.costGold <= player.getGold() && highest.costWood <= player.getWood()) {
	...
	protected List<Request> attack(S3 game, WPlayer player, List<S3Action> actions) {
		...
		for (WUnit fm : footmen) {
			if (fm.getStatus() == null || fm.getStatus().m_action == S3Action.ACTION_STAND_GROUND) {
				nf++;
			}
		}
		...
		Class[] targetClasses = { WCatapult.class, WKnight.class, WArcher.class, WFootman.class, WPeasant.class,
				WTower.class, WTownhall.class, WBarracks.class, WLumberMill.class, WBlacksmith.class, WFortress.class,
				WStable.class };
		...
		if (nf < WAVE_SIZE) {
			for (WUnit u : footmen) {
				if (u.getStatus() != null && u.getStatus().m_action == S3Action.ACTION_ATTACK
					...
					requests.add(new Request(100, u.entityID, 0, 0,
							new S3Action(u.entityID, S3Action.ACTION_ATTACK, enemyTroop.entityID)));
				}
			}
		} else {
			for (WUnit u : footmen) {
	...
	List<Request> buildTroops(S3 game, WPlayer player, List<S3Action> actions) {
		...
		if (troop != null && barracks.getStatus() == null && player.getGold() >= troop.getCost_gold()
				&& player.getWood() >= troop.getCost_wood()) {
	...
	protected List<Request> checkPeasants(S3 game, WPlayer player, List<S3Action> actions) {
		...
		if (gp < nGoldPeasants && freePeasants.size() > 0) {
			...
			if (mine != null) {
				requests.add(new Request(100, peasant.entityID, 0, 0,
						new S3Action(peasant.entityID, S3Action.ACTION_HARVEST, mine.entityID)));
				return requests;
			}
		}
		if (wp < nWoodPEasants && freePeasants.size() > 0) {
			...
			for (WOTree unit : trees) {
				int dist = Math.abs(unit.getX() - peasant.getX()) + Math.abs(unit.getY() - peasant.getY());
				if (dist < leastDist) {
					leastDist = dist;
					tree = unit;
				}
			...
			if (tree != null) {
				requests.add(new Request(100, peasant.entityID, 0, 0,
						new S3Action(peasant.entityID, S3Action.ACTION_HARVEST, tree.getX(), tree.getY())));
				return requests;
			}
		}
		...
		if ((gp < nGoldPeasants || wp < nWoodPEasants) && freePeasants.isEmpty()) {
			...
			if (th != null && (th.getStatus() == null)) {
				requests.add(new Request(100, th.entityID, 400, 0,
						new S3Action(th.entityID, S3Action.ACTION_TRAIN, WPeasant.class.getSimpleName())));
			}
		}
	...
	List<Request> checkBarracks(S3 game, WPlayer player, List<S3Action> actions) {
		...
		if (nb < nBarracks) {
			...
			for (WUnit p : peasants) {
				if (p.getStatus() != null && p.getStatus().m_action == S3Action.ACTION_BUILD
			...
			requests.add(new Request(150, peasant.entityID, 700, 450, new S3Action(peasant.entityID,
					S3Action.ACTION_BUILD, WBarracks.class.getSimpleName(), loc.m_a, loc.m_b)));
		}
		return requests;
	}
	...
	protected List<Request> checkTownhall(S3 game, WPlayer player, List<S3Action> actions) {
		...
		if (null == game.getUnitType(player, WTownhall.class)) {
			...
			requests.add(new Request(200, peasant.entityID, 1200, 800, new S3Action(peasant.entityID,
					S3Action.ACTION_BUILD, WTownhall.class.getSimpleName(), loc.m_a, loc.m_b)));
		}
		return requests;
	}
